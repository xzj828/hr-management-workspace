    ...
    for attempt in range(3):
        try:
            ...
            if attempt == 2 or not _TRANSIENT_CONVERSATION_LIST_ERROR.search(str(exc)):
                raise
            # Conversation listing is read-only. Replaying the same frozen job
            # scope is safe and lets a fresh bridge reconnect after a BOSS SPA
            # document replacement without widening the discovery scope.
            time.sleep(0.4)
    ...
    for sequence, row in enumerate(eligible[:50], start=1):
        ...
        try:
            ...
            if external_id and row.get("unread") and not messages:
                # Opening an unread row marks it read on BOSS. Reopen the same
                # frozen stable ID immediately from the scoped all list rather
                # than relying on a later unread poll that can no longer find it.

...
""")
    ...
    def test_transient_scope_filter_failure_replays_the_frozen_scope(self):
        class Runner:
            def __init__(self):
                self.list_calls = []

            def conversations(self, account, unread=False, job_title=""):
                self.list_calls.append((unread, job_title))
                if len(self.list_calls) == 1:
                    raise BossCliError("未找到 BOSS 沟通“未读”筛选")
                return "1. 林然｜产品经理｜external_id:conversation-safe｜未读 1"

            def open_chat_by_external_id(self, account, external_id, *, job_title="", unread=False):
                return SimpleNamespace(stdout="完整聊天消息：\n[candidate] 2026-08-28 14:30 你好")
        ...
        self.assertEqual(runner.list_calls, [(True, "产品经理"), (True, "产品经理")])
        self.assertEqual(outcome["result"]["list_retry_count"], 1)
        self.assertEqual(outcome["result"]["opened_count"], 1)
        self.assertEqual(outcome["result"]["conversations"][0]["messages"][0]["content"], "你好")

    def test_empty_unread_conversation_reopens_the_same_stable_id_before_success(self):
        class Runner:
            def __init__(self):
                self.opened = []
        ...
        self.assertEqual(runner.opened, [
            ("conversation-safe", "产品经理", False),
            ("conversation-safe", "产品经理", False),
        ])
        self.assertEqual(outcome["result"]["opened_count"], 1)
        self.assertEqual(outcome["result"]["message_retry_count"], 1)
        self.assertEqual(outcome["result"]["message_failed_count"], 0)
        self.assertEqual(outcome["result"]["conversations"][0]["messages"][0]["content"], "你好")

    def test_empty_unread_conversation_fails_closed_after_stable_id_retry(self):
        class Runner:
            def __init__(self):
                self.opened = []
        ...
        self.assertEqual(len(runner.opened), 2)
        self.assertEqual(row["sync_error_code"], "conversation_message_read_failed")
        self.assertNotIn("messages", row)
        self.assertEqual(outcome["result"]["opened_count"], 0)
        self.assertEqual(outcome["result"]["message_retry_count"], 1)
        self.assertEqual(outcome["result"]["message_failed_count"], 1)

    def test_failed_stable_id_reopen_is_counted_and_fails_closed(self):
        class Runner:
            def __init__(self):
                self.open_count = 0
            ...
            def open_chat_by_external_id(self, account, external_id, *, job_title="", unread=False):
                ...
                if self.open_count == 1:
                    return SimpleNamespace(stdout="完整聊天消息：")
                raise BossCliError("BOSS 页面在重开会话时替换了文档")
    ...
    def test_attachment_only_unread_conversation_is_not_misreported_as_message_failure(self):
        class Runner:
            def __init__(self):
                self.open_count = 0
            ...
            def download_resume_attachments(
                ...
                return [{"external_id": "attachment-1", "filename": "resume.pdf"}]
        ...
        row = outcome["result"]["conversations"][0]
        self.assertEqual(runner.open_count, 2)
        self.assertNotIn("sync_error_code", row)
        self.assertEqual(row["attachments"][0]["filename"], "resume.pdf")
        self.assertEqual(outcome["result"]["opened_count"], 1)
        self.assertEqual(outcome["result"]["message_failed_count"], 0)

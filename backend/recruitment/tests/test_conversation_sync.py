...
class ConversationSyncParserTests(SimpleTestCase):
    def test_parses_numbered_conversations_and_unread_state(self):
        ...
        self.assertEqual(rows[0], {"index": 1, "name": "林然", "unread": True, "job_title": "产品经理"})
        self.assertEqual(rows[1], {"index": 2, "name": "周青", "unread": False, "job_title": "测试工程师"})
    ...
    def test_parses_optional_stable_identity_fields(self):
        rows = parse_conversation_list(
            "1. 林然｜产品经理｜external_id:boss-101｜fingerprint:fp_101｜selected:1｜未读 1"
        )

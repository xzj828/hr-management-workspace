...
const title = computed(() => `确认联系 ${props.candidates.length} 位候选人`)
...
<template>
  <RecruitmentDetailDrawer :title="title" @close="emit('close')">
    ...
    <label class="field-label communication-field">沟通动作
    ...
    <label class="field-label communication-field">最终发送内容

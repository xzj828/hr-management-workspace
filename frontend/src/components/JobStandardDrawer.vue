  ...
  <div class="drawer-backdrop" @click.self="emit('close')">
    <aside class="standard-drawer" aria-label="岗位评分标准">
      ...
      <div class="standard-drawer__body">
        ...
        <template v-else>
          ...
          <section class="hard-requirements">
            <header><div><span class="panel-kicker">HARD GATES</span><h3>硬性指标</h3><p>明确反证只会标记为冲突并交给 HR 核实；不会自动改变候选人阶段。</p></div><button v-if="isDraft" class="secondary-button" data-test="add-hard-requirement" @click="addHardRequirement">添加指标</button></header>
            <article v-for="(item, index) in form.hard_requirements" :key="item.key"><span>{{ String(index + 1).padStart(2, '0') }}</span><label><small>指标标识</small><input v-model.trim="item.key" :disabled="!isDraft" placeholder="例如 degree" /></label><label><small>冲突条件</small><input v-model.trim="item.text" :disabled="!isDraft" placeholder="例如：学历低于本科" /></label><button v-if="isDraft" :data-test="`remove-hard-requirement-${index}`" aria-label="删除硬性指标" @click="removeHardRequirement(index)">×</button><div v-if="form.auto_reject_on_hard_fail" class="hard-rule"><label><small>确定性核验字段</small><select v-model="item.rule.field" :disabled="!isDraft" @change="item.rule.operator = ruleOperators(item.rule.field)[0].value; item.rule.value = ''"><option value="total_experience_months">工作经验（月）</option><option value="highest_degree">最高学历</option><option value="skills">技能</option><option value="city">所在城市</option></select></label><label><small>条件</small><select v-model="item.rule.operator" :disabled="!isDraft"><option v-for="operator in ruleOperators(item.rule.field)" :key="operator.value" :value="operator.value">{{ operator.label }}</option></select></label><label><small>阈值（多个用逗号）</small><input v-model="item.rule.value" :disabled="!isDraft" placeholder="例如 36 / 本科 / Java,Vue" /></label></div></article>
            <label v-if="form.hard_requirements.length" class="hard-reject-toggle"><input v-model="form.auto_reject_on_hard_fail" type="checkbox" :disabled="!isDraft" /><span><strong>启用确定性硬性条件核验</strong><small>只标记冲突并记录触发指标与原文证据；需 HR 人工确认，不会自动改变招聘阶段。</small></span></label>
          </section>
      ...
      <footer v-if="standard" class="standard-drawer__footer">
        ...
        <span v-if="isDraft && totalWeight !== 100">权重合计必须为 100 才能启用</span><span v-else-if="isDraft && !hardRulesComplete">硬性条件核验需补全每项确定性规则</span>
      ...
      <div v-if="confirmPublish" class="standard-confirm" data-test="publish-confirm">
        <div><span class="eyebrow">FINAL CHECK</span><h3>启用这份评分标准？</h3><p>启用后内容将锁定，后续简历评分会严格引用此版本。{{ form.auto_reject_on_hard_fail ? '明确违反硬性指标时只会标记冲突并写入审计记录，仍需 HR 确认，不改变招聘阶段。' : 'AI 建议不会自动改变候选人阶段。' }}</p><div><button class="ghost-button" @click="confirmPublish = false">返回检查</button><button class="primary-button" data-test="confirm-publish-standard" :disabled="saving" @click="publish">确认启用</button></div></div>
      </div>
    </aside>
  </div>
</template>

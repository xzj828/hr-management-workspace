...
<template>
  <div class="drawer-backdrop" @click.self="emit('close')">
    <aside class="intelligence-panel" aria-label="简历智能分析">
      ...
      <main class="intelligence-panel__body">
        ...
        <section v-if="activeTab === 'original'" class="intelligence-original">
          ...
          <img v-if="resume.content_type === 'image/png'" :src="resume.preview_url" :alt="`${resume.candidate_name}的原始简历`" />
          <iframe v-else :src="resume.preview_url" :title="`${resume.candidate_name}的原始简历`"></iframe>
        </section>
      ...
      <footer class="intelligence-panel__footer"><span>{{ assessment?.auto_rejected ? '已依据明确反证触发硬性淘汰，并写入审计记录' : '不会自动淘汰或推进候选人' }}</span><button v-if="assessment" class="secondary-button" @click="emit('rescore')">重新评分</button><a class="ghost-button" :href="resume.download_url">下载原文件</a></footer>
    </aside>
  </div>
</template>

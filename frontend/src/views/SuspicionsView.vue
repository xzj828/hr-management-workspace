...
<template>
  <div class="page-stack">
    ...
    <section v-if="currentBatch" class="review-explainer"><div class="review-explainer__mark"><AppIcon name="alert-circle" :size="18" /></div><div><strong>本批次识别到 {{ currentBatch.suspicion_count }} 条疑似记录</strong><p>选择“归前一天”时，凌晨记录不计入当天；选择“保留当天”时，当天增加一个有效出勤日。</p></div></section>
    ...
    <section v-else class="review-list">
      <article v-for="row in rows" :key="row.id" class="review-card">
        ...
        <div class="review-card__timeline">
          ...
          <i class="timeline-arrow"><AppIcon name="arrow-right" :size="18" /></i>
      ...
      <div v-if="!rows.length" class="empty-state"><div class="empty-state__icon"><AppIcon name="check-circle" :size="29" /></div><strong>没有待审核的跨日记录</strong><p>本批次的疑似记录已经处理完毕。</p></div>
    </section>
  </div>
</template>
